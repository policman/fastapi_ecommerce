...
class UserCreate(BaseModel):
    ...
    role: str = Field(
        default="buyer", pattern="^(buyer|seller)$", description="Роль buyer/seller"
    )
...
class ReviewCreate(BaseModel):
    comment: str | None = Field(None, description="Текст отзыва. Может отсутствовать")
    grade: int = Field(ge=1, le=5, description="Оценка пользователя 1-5")
